...
if [[ $1 =~ .bam$ ]] || [[ $1 =~ .cram$ ]]; then
  samtools view -h -@$cpu $1 | ./k8 ./paftools.js sam2paf - | cut -f1-16 - > $2
else
  ## Add -L for long cs tag form
  ./k8 ./paftools.js sam2paf $1 | cut -f1-16 - > $2
...
set -x
java -jar -Xmx4g ./pafToCovClippedWig.jar $2 "$name" 1024 $out_prefix
