#!/usr/bin/env python3
"""
Generate per-chromosome coding cov vs. E1 scatter plots and genome-wide correlation summaries
for dip, mat+Z, and pat+WZ datasets.

Run from the directory containing your *.cis.vecs.tsv and *.coding_cov.bedgraph files.  
...
def make_plots():
    ...
    for vecs_path in vecs_files:
        ...
        coding_path = f"bTaeGut7.{hap}.cur.20250313.bwa.100000.coding_cov.bedgraph"
        if not os.path.exists(coding_path):
            print(f"[!] Missing coding track: {coding_path}, skipping {hap}")
            continue
        ...
        df = pd.merge(
            ...
            on=["chrom", "start", "end"],
            how="inner",
        ).dropna(subset=["E1", "coding"])

        # compute per-chrom correlations and scatter plots
        corrs = {}
        for chrom, grp in df.groupby("chrom"):
            r = grp["E1"].corr(grp["coding"])
            corrs[chrom] = r

            fig, ax = plt.subplots(figsize=(4, 3))
            ax.scatter(grp["coding"] * 100, grp["E1"], s=5, alpha=0.5)
            ax.axhline(0, color="gray", linewidth=1)
            ax.set_title(f"{hap} · {chrom} (r={r:.2f})")
            ax.set_xlabel("Coding content (%)")
            ax.set_ylabel("E1")
            fig.tight_layout()
        ...
        ax.set_ylabel("Pearson r (E1 vs Coding)")
        ax.set_title(f"{hap} genome-wide E1/Coding correlation")
        fig.tight_layout()
